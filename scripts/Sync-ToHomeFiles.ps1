<#
.SYNOPSIS
  Syncs tracked files from this repo into the home directory.

.DESCRIPTION
  Reads a manifest of home-relative file paths and copies each existing repo
  file into the matching path under the current user's home directory. If a
  listed repo file is missing, the matching home file can be removed after an
  explicit confirmation.

.EXAMPLE
  .\Sync-ToHomeFiles.ps1

.EXAMPLE
  .\Sync-ToHomeFiles.ps1 -WhatIf
#>
...
foreach ($entry in $entries) {
    $sourcePath = Resolve-TrackedPath -BasePath $repoRoot -RelativePath $entry -Label 'repo'
    $destinationPath = Resolve-TrackedPath -BasePath $homeRoot -RelativePath $entry -Label 'home'
    ...
    if (Test-Path -LiteralPath $sourcePath -PathType Leaf) {
        if ((Test-Path -LiteralPath $destinationPath -PathType Leaf) -and (Test-FileContentMatch -PathA $sourcePath -PathB $destinationPath)) {
            ...
            Add-Result -Results ([ref]$results) -Path $entry -Status 'Unchanged' -Detail 'Repo and home files already match'
            continue
        }
        ...
        if ($PSCmdlet.ShouldProcess($destinationPath, 'Copy tracked file from repo')) {
            Copy-Item -LiteralPath $sourcePath -Destination $destinationPath -Force
            Write-Ok "Synced $entry"
            Add-Result -Results ([ref]$results) -Path $entry -Status 'Synced' -Detail 'Copied from repo into home directory'
        }
        else {
            Write-Warn "Skipped $entry"
    ...
    if (Test-Path -LiteralPath $sourcePath) {
        throw "Manifest entry '$entry' is not a file in the repo"
    }

    if (Test-Path -LiteralPath $destinationPath -PathType Leaf) {
        if (-not $PSCmdlet.ShouldProcess($destinationPath, 'Delete home copy because repo source file is missing')) {
            Write-Warn "Skipped $entry"
            Add-Result -Results ([ref]$results) -Path $entry -Status 'Skipped' -Detail 'Deletion declined or previewed'
            continue
        }

        $confirmationMessage = "Repo source for '$entry' is missing.`nDelete '$destinationPath' from the home directory?"
        if (-not $PSCmdlet.ShouldContinue($confirmationMessage, 'Confirm delete')) {
            Write-Warn "Kept $entry"
            Add-Result -Results ([ref]$results) -Path $entry -Status 'Skipped' -Detail 'Repo source missing; user declined deletion'
            continue
        }

        Remove-Item -LiteralPath $destinationPath -Force
        Write-Ok "Removed $entry"
        Add-Result -Results ([ref]$results) -Path $entry -Status 'Removed' -Detail 'Repo source missing and home file deleted after confirmation'
        continue
    }

    Write-Warn "Missing source for $entry"
    Add-Result -Results ([ref]$results) -Path $entry -Status 'Skipped' -Detail 'Source file missing from repo and no home copy to delete'
}
