...
for entry in "${entries[@]}"; do
  source_path="$(resolve_tracked_path "$home_root" "$entry" 'home')"
  destination_path="$(resolve_tracked_path "$repo_root" "$entry" 'repo')"

  if [[ -f "$source_path" ]]; then
    if [[ -f "$destination_path" ]] && cmp -s -- "$source_path" "$destination_path"; then
      write_ok "Unchanged $entry"
      add_result "$entry" 'Unchanged' 'Home and repo files already match'
      continue
    fi
    ...
    add_result "$entry" 'Synced' 'Copied from home directory'
    continue
  fi

  if [[ -e "$source_path" ]]; then
    die "Manifest entry '$entry' is not a file in the home directory"
  fi
  ...
  if [[ -f "$destination_path" ]]; then
    ...
    printf "Home source for '%s' is missing.\nDelete '%s' from the repo? [y/N] " "$entry" "$destination_path"
    read -r confirmation
    case "$confirmation" in
      y | Y | yes | YES)
        rm -f -- "$destination_path"
        write_ok "Removed $entry"
        add_result "$entry" 'Removed' 'Home source missing and repo file deleted after confirmation'
        ;;
      *)
        write_warn "Kept $entry"
        add_result "$entry" 'Skipped' 'Home source missing; user declined deletion'
        ;;
    esac
    continue
  fi
  ...
  add_result "$entry" 'Skipped' 'Source file missing and no repo copy to delete'
done
